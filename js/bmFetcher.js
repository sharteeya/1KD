...
function getAnalysisData(tid, uid){
    ...
    if(window.location.host == "1know.net") xhr.open("GET", `//1know.net/private/group/task/${tid}/analytics/unit/${uid}`);
    else xhr.open("GET", `//www.1know.net/private/group/task/${tid}/analytics/unit/${uid}`);
...
function getMember() {
    ...
    if(window.location.host == "1know.net") xhr.open("GET",`//1know.net/private/group/${GROUP_ID}/member`);
    else xhr.open("GET",`//www.1know.net/private/group/${GROUP_ID}/member`);
    xhr.onload = function() {
        let data = JSON.parse(xhr.responseText);
        MEMBER_DATA = data;
        data.map((student, i) => {
...
function getTasks() {
    let xhr = new XMLHttpRequest();
    xhr.withCredentials = true;
    if(window.location.host == "1know.net") xhr.open("GET", `//1know.net/private/group/${GROUP_ID}/task`);
    else xhr.open("GET", `//www.1know.net/private/group/${GROUP_ID}/task`);
    xhr.onload = function() {
        let data = JSON.parse(xhr.responseText);
        TASK_LIST = data;
...
function getGroup(){
    let xhr = new XMLHttpRequest();
    if(window.location.host == "1know.net") xhr.open("GET", `//1know.net/private/group/${GROUP_ID}/member/team`);
    else xhr.open("GET", `//www.1know.net/private/group/${GROUP_ID}/member/team`);
    xhr.onload = function(){
        let data = JSON.parse(xhr.responseText);
        data.map((g,i) => {
